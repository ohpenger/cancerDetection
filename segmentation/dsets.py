...
candidate_with_malignent_file_path = "CSVFILES\\annotations_with_malignancy.csv"
...
def getCandidateInfoList(requireOnDisk_bool=True):
    ...
    # the subsets yet.
    mhd_list = glob.glob(os.path.join(base_path,'subset0*\*.mhd'))
    presentOnDisk_set = {os.path.split(p)[-1][:-4] for p in mhd_list}

    candidateInfo_list = []
    with open(os.path.join(base_path,candidate_with_malignent_file_path), "r") as f:
    ...
    # iteration = iter(candidateInfo_list)
    # while True:
    #     try:
    #         candidateInfoTuple = next(iteration)
    #         series_uid = candidateInfoTuple.series_uid
    #         mhd_path = glob.glob(
    #             'E:\luna16\subset*/{}.mhd'.format(series_uid)
    #         )
    #         if not mhd_path:
    #            candidateInfo_list.remove(candidateInfoTuple)
    #     except StopIteration:
    #         pass
    # for candidateInfoTuple in candidateInfo_list:
    #     series_uid = candidateInfoTuple.series_uid
    #     mhd_path = glob.glob(
    #         'E:\luna16\subset*/{}.mhd'.format(series_uid)
    #     )
...
class Ct:
    def __init__(self, series_uid):
        mhd_path = glob.glob(
            'E:\luna16\subset0*/{}.mhd'.format(series_uid)
        )[0]
